            ...
            while(!input.eof()){
                if(line.length())
                    ...
                    if(temp == "DEFINITION"){
                        ...
                        temp = regex_replace( line.substr(line.find_first_of(" "), line.length()), allowed, " " );
                        keywords = source_split(temp);
                        for(set<string>::iterator word = keywords.begin(); word != keywords.end(); ++word )
                            definitions.insert(*word);
                    }
                    else if(temp == "SOURCE"){
                        line = line.substr(line.find_first_of(" "), line.length() );
...
int main(int argc, char** argv){
        ...
        if (argc >= 7){
            thread_vector.push_back(thread(output_gi_key_src, argv[6]));

            for (auto& thread : thread_vector) {
                thread.join();
            }
            thread_vector.clear();
            thread_vector.push_back(thread(sort_file, argv[6] ));
        }

        vector<thread> compression_threads;
        vector<string> key_words_sorted(key_words.begin(), key_words.end());
        key_words.clear();
        swap(empty_set, key_words);
        sort(key_words_sorted.begin(), key_words_sorted.end());

        thread_vector.push_back(thread(output_db, argv[3], ref(key_words_sorted) ));

        vector<string> source_words_sorted(source_words.begin(), source_words.end());
        source_words.clear();
        swap(empty_set, source_words);

        sort(source_words_sorted.begin(), source_words_sorted.end() );

        thread_vector.push_back(thread(output_source, argv[4], ref( source_words_sorted)));
